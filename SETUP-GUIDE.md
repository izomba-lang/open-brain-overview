...
| Скилл | Что делает |
|-------|-----------|
| `text-quality-pipeline` | Пайплайн качества текста: анти-слоп, инфостиль, JTBD, тон. Применяется ко всем текстам от лица пользователя |
| `draft-email` | Пишет черновик письма/текста в стиле пользователя с контекстом из базы |
| `email_writer` | Генерация текстов (письма, посты, гайды) с пайплайном качества |
| `follow-up` | Формирует письмо-фоллоуап после встречи с итогами и следующими шагами |
...
| `weekly-report` | Еженедельный отчёт из заметок за неделю, группировка по проектам |
| `delegate-task` | Составляет чёткую постановку задачи для делегирования конкретному человеку |
| `decision-log` | Фиксирует решение с контекстом, альтернативами и обоснованием |
| `product-manager` | Полный цикл управления продуктом: стратегия, дорожная карта, метрики |
| `plan-ceo-review` | Обзор плана для CEO — переосмысление проблемы и поиск амбициозного решения |
| `executive-summary-generator` | Преобразует сложные бизнес-данные в краткие резюме для руководителей |
| `weekly-engineering-retrospective` | Анализирует историю коммитов, рабочие паттерны и метрики качества кода |

**Разработка и анализ**

| Скилл | Что делает |
|-------|-----------|
| `code-reviewer` | Рецензирование кода: корректность, поддерживаемость, безопасность, производительность |
| `office-hours` | YC-стиль мозговой штурм: режим стартапа (6 вопросов) или режим строителя |
| `expense-report-aed` | Отчёт о расходах из чеков такси/перелётов с конвертацией в AED |
