...
## 🚶 Рекомендуемый способ: скилл-проводник для Claude Code

В репо есть скилл [`skills/open-brain-companion`](skills/open-brain-companion/) — интерактивный проводник. Вместо того чтобы читать 500-строчный гайд и ковыряться в нём — поставь скилл и Claude Code проведёт тебя пошагово, по одному шагу за раз, с проверками на каждом этапе.

**Установка (30 секунд):**
```bash
git clone https://github.com/izomba-lang/open-brain-overview.git
mkdir -p ~/.claude/skills
cp -r open-brain-overview/skills/open-brain-companion ~/.claude/skills/
```

Потом открой Claude Code и напиши: **"Помоги настроить Open Brain"**. Скилл подхватится автоматически и спросит с чего начать.

Если предпочитаешь делать всё сам — читай гайд ниже.
